...
class TV:
    def __init__(self, canal, minimo, maximo):
        self.ligada = False
        self.canal_inical = canal
        self.canal_minimo = minimo
        self.canal_maximo = maximo

    def muda_canal_para_baixo(self):
        if self.canal_inical - 1 >= self.canal_minimo:
            self.canal_inical -= 1

    def muda_canal_para_cima(self):
        if self.canal_inical + 1 <= self.canal_maximo:
            self.canal_inical += 1
...
print(f'Canal inicial: {TV_da_sala.canal_inical}\n')

for x in range(0, 120):
    TV_da_sala.muda_canal_para_cima()

print(f'Canal máximo após subir 120x: {TV_da_sala.canal_inical}')

for x in range(0, 120):
    TV_da_sala.muda_canal_para_baixo()

print(f'Canal mínimo após descer 120x: {TV_da_sala.canal_inical}')
